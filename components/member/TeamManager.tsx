...
import { PartyPopper, Share2, MapPin } from "lucide-react";
      ...
      <div className="space-y-3">
        {justJoined && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-4 text-center space-y-2">
            <p className="flex items-center justify-center gap-2 font-bold text-green-900">
              <PartyPopper className="w-5 h-5 shrink-0" aria-hidden="true" />
              Te voilà dans l&apos;équipe {team.name} !
            </p>
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          ...
          {zones.map((z) => (
            <span key={z} className="inline-flex items-center gap-1 text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full font-medium">
              <MapPin className="w-3 h-3 shrink-0" aria-hidden="true" />
              {z}
            </span>
          ))}
