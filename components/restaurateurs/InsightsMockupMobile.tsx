...
import { useScrollScrub } from "./motion";

// Version mobile-native du mockup "Opportunités" — mêmes chiffres que
// l'ancien InsightsMockup (desktop), recomposés à des tailles/paddings
// mobiles. Hauteur d'écran fixe (proportion téléphone réel) avec défilement
// interne synchronisé au scroll de la page (useScrollScrub), même mécanique
// que MemberAppMockup — sinon le cadre du téléphone s'étire à l'infini avec
// le contenu au lieu de garder une taille réaliste.
export function InsightsMockupMobile() {
  const { viewportRef, contentRef } = useScrollScrub();

  return (
    <div className="bg-paper rounded-[25px] overflow-hidden">
      <div ref={viewportRef} className="relative h-[480px] sm:h-[520px] overflow-hidden">
        ...
        {/* Même indice de défilement que MemberAppMockup — signale qu'il y a
            plus de contenu sous la ligne de flottaison du téléphone. */}
        <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-10 bg-gradient-to-t from-paper to-transparent" />
      </div>
    </div>
  );
}
