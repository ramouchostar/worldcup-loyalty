-- ============================================================
-- 20260914-2010 — La réserve en points courbés, version corrigée (ADR 0060)
--
-- REMPLACE 20260914-1932-reserve-points-courbes.sql, qui n'a jamais été
-- appliquée : son exécution a échoué le 2026-09-14 (23505 sur
-- reward_tiers_restaurant_id_layer_min_threshold_key, établissement démo
-- zz-test-sushi-34) et la transaction a tout annulé. Ne PAS exécuter
-- l'ancien fichier ; exécuter celui-ci.
--
-- CAUSE DE L'ÉCHEC
--   La conversion des seuils de réserve était un UPDATE ensembliste.
--   PostgreSQL vérifie l'unicité (restaurant, couche, seuil) ligne par ligne :
--   le seuil 150 converti en 400 heurtait le seuil 400 pas encore converti.
--   Correctif (bloc 5b) : conversion une ligne à la fois, dans l'ordre qui ne
--   croise jamais un seuil non converti (du plus grand au plus petit quand la
--   conversion agrandit, l'inverse quand elle rétrécit) ; deux seuils qui
--   tombent sur la même valeur après arrondi → le second est désactivé.
--
-- QUOI
...
--      gros cadeau de réserve, le seuil (en points courbés) converti en
...
--   côté » montre déjà les points courbés.
--
-- Idempotente : fonctions en CREATE OR REPLACE ; données protégées par le
-- marqueur. Rejouable sans effet. Tout ou rien (BEGIN … COMMIT).
-- ============================================================
...
BEGIN
  ...
  --     arrondis à 5. Une ligne à la fois : quand la conversion agrandit
  --     les seuils (facteur ≥ 1, cas courant), du plus grand au plus petit ;
  --     quand elle les rétrécit, du plus petit au plus grand. Ainsi un seuil
  --     converti ne croise jamais un seuil pas encore converti. Deux seuils
  --     qui tombent sur la même valeur après arrondi : le second garde son
  --     ancienne valeur et est désactivé (invisible des membres et de la
  --     console), plutôt que de faire échouer toute la migration.
  FOR v_tier IN
    SELECT t.id, t.restaurant_id, t.min_threshold,
           points_for_order(average_basket(t.restaurant_id)) / average_basket(t.restaurant_id) AS factor
    FROM reward_tiers t
    WHERE t.layer = 'saver'
    ORDER BY t.restaurant_id,
             CASE WHEN points_for_order(average_basket(t.restaurant_id)) >= average_basket(t.restaurant_id)
                  THEN -t.min_threshold ELSE t.min_threshold END
  LOOP
    v_new := GREATEST(5, round(v_tier.min_threshold * v_tier.factor / 5) * 5);
    IF v_new <> v_tier.min_threshold THEN
      BEGIN
        UPDATE reward_tiers SET min_threshold = v_new WHERE id = v_tier.id;
      EXCEPTION WHEN unique_violation THEN
        UPDATE reward_tiers SET is_active = false WHERE id = v_tier.id;
      END;
    END IF;
  END LOOP;
