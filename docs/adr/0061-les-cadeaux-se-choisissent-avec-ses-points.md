...
## Amendement — taux cadeaux par établissement (2026-09-19)

- Le propriétaire de Kraainem trouvait les cadeaux trop généreux (Magnifique Beef Menu
  pour 51 € de tickets) et voulait que ses clients dépensent **deux fois plus**. Prix =
  coût ÷ taux × 10 : doubler la dépense, c'est **diviser le taux par deux** → **4 %**
  (Beef Menu : 1 020 points ≈ 102 € de tickets). 16 % aurait fait l'inverse.
- Décision du porteur : **toute la logique de Kraainem passe à 4 %** — prix du catalogue,
  plafond mensuel (ADR 0012), couverture des cadeaux d'équipe (ADR 0017), plafonds de
  paliers, cadeau 4 jetons, anniversaire. Le taux vit dans
  `restaurant_reward_settings.budget_pct` (durable ; recopié sur le mois de budget en
  cours et les suivants) ; côté code, `getRestaurantBudgetPct` est la seule source.
  Sans réglage : 8 %.
