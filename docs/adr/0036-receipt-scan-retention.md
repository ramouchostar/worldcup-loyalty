...
Une seule exception, tenue à la main : le préfixe `echantillons/` du bucket, que la rétention ne balaie jamais. On y range les quelques tickets qui servent de **référence de contrôle OCR** — dissociés de leur membre (plus de `user_id` dans le chemin), qu'aucune ligne ne référence, gardés sciemment pour juger la lecture du modèle dans la durée. Rien ne s'y dépose automatiquement ; y copier un ticket est une décision, pas un effet de bord.

La purge fait trois passes, dont la troisième balaie ce que **plus aucune ligne ne référence**. Sans elle, une image dont la commande a été supprimée — test, doublon nettoyé, effacement RGPD antérieur — resterait dans le bucket indéfiniment : au premier inventaire (2026-08-20), **7 fichiers sur 8** étaient exactement dans ce cas. Le balayage ne considère que les fichiers de plus de 30 jours, ce qui exclut toute course avec une image qui vient d'être déposée.
