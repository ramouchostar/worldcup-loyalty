...
- **ADR 0067 — les tailles ne sont pas des plats** : « Nuggets (4PC.) » sur le ticket = « Nugget (4) » sur la carte. Toujours comparer via `lib/menu-quantity.ts` (plat sans taille + taille exacte), jamais le libellé brut ; une taille absente est une taille manquante, pas un nouveau plat. Affichage membre : « 6 Churros » (`displayItemName`), jamais un renommage en base
