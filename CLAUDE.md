...
- **ADR 0067 — les tailles ne sont pas des plats** : « Nuggets (4PC.) » sur le ticket = « Nugget (4) » sur la carte. Toujours comparer via `lib/menu-quantity.ts` (plat sans taille + taille exacte), jamais le libellé brut ; une taille absente de la carte n'est JAMAIS suggérée : c'est n fois l'unité (rattachée à l'article « (1) », quantité multipliée, prix ramené à l'unité). Affichage membre : « 6 Churros » (`displayItemName`), jamais un renommage en base
- **ADR 0066 — la carte d'identité du ticket** : chaque lecture conserve date imprimée, heure, canal, numéro du jour, sous-total, remise, moyen de paiement (jamais de donnée bancaire, ADR 0025) et le résultat des contrôles nommés (`lib/receipt-coherence.ts`). **Phase 1 : mesuré, ne refuse rien** — une lecture peut INVENTER des valeurs plausibles (terrain 2026-09-20 : deux commandes validées pour un seul ticket), donc comparer des valeurs ne suffit pas
...
- Catalogue (`/r/[id]/points`, `/reserve` redirige) : `catalog_items`, prix calculé `catalog_price_points` (arrondi 5 sup de coût ÷ taux × 10 ; taux cadeaux par établissement `restaurant_reward_settings.budget_pct`, réglé par le gérant entre 4 et 12 % — ADR 0068, page console « Taux cadeaux » ; baisser le taux RENCHÉRIT les cadeaux, et `set_reward_pct` ajuste les soldes des clients dans la même transaction), échange `exchange_points_for_item`. Jamais de prix de revient côté membre
...
- Séquences membres : règles pures dans `lib/sequence-rules.ts` (fenêtre de 3 jours par étape — jamais de rattrapage massif, témoin stable par hachage), moteur `lib/sequence-runner.ts`, cron `/api/cron/sequences` ; tout e-mail de séquence porte un lien d'arrêt `/e/stop/<envoi>` et l'en-tête `List-Unsubscribe` (arrêts dans `message_optouts`, **fail-closed** : sans la table, rien ne part)
