...
const CONFIDENCE: Record<"low" | "medium" | "high", { label: string; cls: string }> = {
  low: { label: "Confiance faible", cls: "bg-warn/12 text-warn" },
  medium: { label: "Confiance moyenne", cls: "bg-paper-subtle text-ink-body" },
  high: { label: "Confiance élevée", cls: "bg-good/12 text-good" },
};
...
export default async function ForecastPage({
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={<>Prévisions de chiffre d&apos;affaires</>}
        subtitle={<>Ce que ta caisse va probablement faire dans les 7 prochains jours — calculé sur tes ventes importées,
      ...
      {nonDaily && trend ? (
          ...
          <div className="bg-white rounded-xl border border-paper-border p-5">
            ...
            <div className="mt-3 grid sm:grid-cols-3 gap-3">
              <div className="bg-paper rounded-xl p-3">
                ...
                <p className="text-xl font-black text-ink tabular-nums">{euro0(trend.avgRecent)}</p>
              </div>
              <div className="bg-paper rounded-xl p-3">
                <p className="text-xs text-ink-muted">Dernière {trend.periodLabel}</p>
                <p className="text-xl font-black text-ink tabular-nums">{trend.last !== null ? euro0(trend.last) : "—"}</p>
              </div>
              <div className="bg-paper rounded-xl p-3">
                <p className="text-xs text-ink-muted">vs la précédente</p>
                <p className={`text-xl font-black tabular-nums ${trend.direction === "up" ? "text-good" : trend.direction === "down" ? "text-danger" : "text-ink"}`}>
        ...
        <div className="bg-white rounded-xl border border-dashed border-paper-border p-8 text-center">
          <p className="text-lg font-bold text-ink">Pas encore assez de données</p>
          <p className="text-sm text-ink-muted mt-2 max-w-md mx-auto">{forecast.message}</p>
        ...
        <>
          ...
          <div className="bg-ink text-white rounded-xl p-5">
            ...
            <p className="text-xs text-ink-faint mt-3">{forecast.confidenceReason}</p>
          </div>
    ...
    <div className="bg-white rounded-xl border border-paper-border p-4">
      <div className="flex items-baseline justify-between">
        <p className="font-semibold text-ink">
          {day.weekdayLabel} <span className="text-ink-faint font-normal">{dd}/{m}</span>
        </p>
        {day.closed ? (
          <span className="text-xs text-ink-faint">habituellement fermé</span>
        ) : (
          <p className="text-lg font-black text-ink tabular-nums">{euro0(day.expected)}</p>
        )}
      </div>
      {!day.closed && (
        <>
  ...
  return (
    <span
      className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${
        up ? "bg-good/10 text-good" : "bg-danger/10 text-danger"
      }`}
