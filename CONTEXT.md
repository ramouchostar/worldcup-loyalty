...
Encaissement enregistré par le système de caisse du restaurateur (membre ou non), importé par CSV/Excel à part de `orders`. **Jamais affichée ligne à ligne** (le resto a le détail dans sa caisse) — c'est la matière brute du **forecast de CA total**. Ce qui EST montré, c'est la prévision qui en découle, réservée à l'**admin/restaurateur** (jamais côté client/membre — c'est la vue admin, qui a toujours le droit aux euros ; l'ADR 0007 ne vise que le membre). Table `restaurant_sales`, **service-role only**. Une **commande directe** est une vente caisse qui a *en plus* été scannée par un membre. Le rapport est **déposé tel quel** (2026-08-22) : colonnes devinées par en-têtes + contenu (`guessMappingSmart`), **granularité détectée** (`detectGranularity` : jour / semaine / mois) — détail journalier → prévision 7 jours ; totaux hebdo/mensuels → **lecture de tendance** (moyenne récente, dernière période, direction), jamais de prévision jour par jour inventée.
...
**Taux cadeaux** *(ADR 0068)* :
La part des encaissements du programme qu'un établissement rend en cadeaux — **4 à 12 %**, réglée par son gérant (console, page « Taux cadeaux »), 8 % par défaut. Elle pilote TOUT : prix du catalogue en points, plafond mensuel (ADR 0012), couverture des cadeaux d'équipe (ADR 0017), plafonds de paliers, cadeau 4 jetons, anniversaire. Se lit toujours via `getRestaurantBudgetPct`. Baisser le taux RENCHÉRIT les cadeaux : les soldes des clients sont ajustés dans la même transaction (`set_reward_pct`) pour que personne ne perde. Montré au restaurateur en **vitesse** (« Beef Menu : 6 tickets ») et en **coût** (« au plus 49 € ce mois-ci ») — jamais en promesse d'acquisition.
_Avoid_ : « remise » ou « pourcentage de réduction » (le client ne voit jamais d'euros), promettre un gain de clients.
...
Tout bouton ticket ouvre **directement la caméra** (ADR 0057) ; la croix ramène à l'écran d'avant. Une photo **incomplète** (total ou clé illisible, année de la clé réparée) se **reprend** : le message dit quoi recadrer — jamais de saisie à la main (ADR 0058). La photo se prend **dans l'app** (vue caméra plein écran, cadre total + clé de commande — ADR 0056) ; l'appareil photo du téléphone et la galerie restent le repli. Tout message (photo refusée, erreur d'envoi, doublon) s'affiche **en haut de l'écran**, jamais sous la ligne de flottaison.
_Avoid_ : scanner / scan (côté client, action ticket) ; icône viseur lucide `Scan` sur une surface client ; un message d'erreur qu'il faut scroller pour voir.

**Envoi automatique du ticket** *(ADR 0055)* :
Après la photo, un ticket **bien lu** (montant accepté, numéro lu, année non réparée, pas de doublon) part tout seul : « Vérification en cours... » puis le résultat, sans tap. **Aucun récap, aucune saisie** (ADR 0058) : le montant et le numéro ne se modifient pas ; le serveur relit la photo et n'utilise que sa propre lecture. Une lecture incomplète (total ou numéro absent, année du numéro réparée) se reprend en photo. La confirmation « commande passée directement au restaurant, pas via une plateforme » accompagne la photo (« En prenant la photo, tu confirmes… »). La photo reste sur l'appareil jusqu'à la réponse du serveur, visiteur comme membre.
_Avoid_ : toute saisie ou correction du montant ou du numéro par le membre (ADR 0058) ; un bouton d'envoi qu'il faut scroller pour atteindre ; traiter la photo prise comme la fin du parcours (rien n'est gagné tant que le serveur n'a pas le ticket).
...
Suite de messages déclenchée par l'état d'un membre ou d'un établissement, écrite une fois et jouée par la plateforme : « Ton premier ticket », « Rejoins ton équipe », « Invite tes amis », « Installe l'app » côté membres ; « Cap franchi », « Ta semaine », « L'idée de la semaine » côté restaurateurs. Chaque séquence a un plafond, un arrêt propre (« Ne plus recevoir ces rappels » — coupe cette séquence pour cette personne, jamais le programme ni les informations de service) et un seul critère de réussite mesuré sur 7 jours. Une étape n'est due que dans les 3 jours qui suivent sa date : allumer une séquence ne relance pas les inscrits d'avant. **Éteinte par défaut**, allumée par établissement depuis `/platform`. Distincte du broadcast (composé à la main) et des notifications d'incitation (ADR 0009, déclenchées par le score d'équipe).
...
**Onboarding visiteur** *(ADR 0040, assoupli par ADR 0045, converti par ADR 0048, prolongé par ADR 0049)* :
La valeur d'abord, le compte au premier ticket. Un visiteur (QR scanné, pas de compte) arrive sur la vitrine (le tour de bienvenue en modal a été retiré — ADR 0044), navigue librement, photographie son ticket — l'aperçu OCR tourne tout de suite (non authentifié, bridé par IP, ADR 0045) et affiche **l'écran de gain** (ci-dessous) — et c'est à l'ENVOI que le compte est demandé (« réclame ton cadeau », Google en un tap d'abord). La photo attend sur l'appareil (IndexedDB, 24 h) et la soumission reprend seule après connexion (`?resume=1`). Le cadeau continue de payer les demandes suivantes — app, notifications, équipe (voir **Feuille post-ticket**). Un membre qui rescanne le QR arrive directement sur son dashboard.
_Avoid_ : mode invité (pas de points en suspens sur session anonyme), mur de connexion, « garde tes points » comme argument du compte quand un cadeau est atteint (c'est le cadeau qui paie la demande — ADR 0048).

**Écran de gain** *(ADR 0048)* :
L'état « photo lue » de l'écran ticket côté visiteur (`TicketGainCard`) — **l'écran de conversion du parcours**. Dit ce que le ticket VAUT avant que quoi que ce soit ne soit demandé, dans cet ordre : les **points** en très gros, puis le **cadeau de couche 1 nommé et illustré** (« Ton cadeau : X · À récupérer au comptoir ») ou, sous le premier palier, la **distance nommée** au prochain (« Prochain cadeau — Y » + barre), puis le **montant relu** en petit (« Lu sur ton ticket : 36,10 € »). Couche 1 seulement : c'est la seule due sans équipe (ADR 0034) et la seule qu'aucun verrou invisible ne coupe. Séquence du parcours : **gain → compte → app → notifications → équipes**, chaque demande payée par le cadeau déjà gagné.
_Avoid_ : « Scan réussi » / « Montant détecté » (l'ancien libellé — une preuve technique, pas un argument), « points gagnés » (dire « points sur ce ticket » : rien n'est crédité avant validation), « validé »/« vérifié »/« automatique »/« instantané » (ADR 0008), tout seuil ou écart chiffré vers le palier suivant (ADR 0007/0028).

**Feuille post-ticket** *(ADR 0049, complète ADR 0038)* :
La feuille (`PostTicketSheet`) qui propose **l'app puis les notifications** sur l'écran de résultat du ticket — donc **après le compte ET après la soumission** : sur iOS, ajouter le site à l'écran d'accueil crée un conteneur de stockage séparé de Safari, où la photo en attente (IndexedDB) ne suit pas. Tant que le serveur n'a pas le ticket, quitter Safari le perd. Chaque ligne est **payée par le cadeau déjà gagné** — « pour **récupérer** ton {cadeau} à ta prochaine visite », « pour **savoir quand** il t'attend » — et retombe sur un libellé neutre quand aucun cadeau n'est nommé. Portée : **une fois par visite** (clé `sessionStorage`), plus une fois par appareil ; « Plus tard » et un tap sur le fond ne valent que pour la visite, seule une réponse explicite (dialogue natif annulé, ou « C'est fait ✅ » sur le chemin manuel iOS) est définitive. Passe **avant** la question d'équipe, et la libère même quand elle ne s'ouvre pas. La **carte d'installation permanente** (ADR 0038) reste le rattrapage et n'est jamais silencée par un refus.
_Avoid_ : « une fois par appareil » (c'était le bug — un tap à côté consommait la seule proposition du parcours) ; proposer l'app avant le compte ou avant l'envoi du ticket (perte de ticket sur iOS) ; « Et maintenant ? » comme titre (un menu, pas un argument) ; interrupteur ou accent rouge sur cet écran (ADR 0048 §7).

**Carte de la vitrine** *(ADR 0062, remplace les ADR 0042/0043)* :
La carte « Ce que ton ticket te rapporte » de la vitrine publique (`/r/[id]`, cible des QR) : le **cadeau d'accueil** nommé (« {article} offert — pour ton premier ticket »), **des points à chaque ticket** avec trois articles du catalogue (photo et prix en points, repères ≈ 1, 3 et 6 tickets moyens), et **des cadeaux d'équipe** là où l'établissement a des paliers d'équipe. Jamais d'euro ni de taux de points.
_Avoid_ : un article tiré au hasard par tranche de prix (ancien aperçu), promettre des points sans ticket.
...
Appel à l'action principal de la landing restaurateurs (`/audit-gratuit`) : le prospect choisit son établissement (suggestions Google, 19 communes de Bruxelles), voit l'**analyse en direct** (six étapes cochées quand leur donnée est arrivée, ses vrais voisins, avis et photos sous la ligne verte), reçoit une **note rapide** sur 100, puis laisse son numéro WhatsApp pour recevoir le **rapport complet** (l'audit de l'ADR 0069, relu par l'équipe). Chaque analyse est une **demande** (`audit_leads`), gardée même sans numéro.
