...
_Avoid_ : propriétaire. (« gérant » n'est plus à éviter — c'est un rôle établissement défini, voir **Rôle établissement**, ADR 0040.)

**Rôle établissement** *(ADR 0040)* :
...
**Lien d'invitation restaurateur** *(ADR 0032, rôle ajouté par ADR 0040)* :
...
_Avoid_ : magic link (réservé à la connexion Supabase), invitation membre (le parrainage, ADR 0006, est un autre objet), « évince »/« remplace » (une invitation ordinaire n'évince plus personne depuis ADR 0040 — seul un futur transfert de gérance, non construit, le ferait).
...
_Avoid_ : superuser ; « manager »/« gérant » seuls sans préciser le rôle établissement (ADR 0040 — voir **Rôle établissement**, un objet différent de ce pont legacy).
